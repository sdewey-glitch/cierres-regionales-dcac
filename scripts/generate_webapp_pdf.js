...
const MD_PATH = path.join(__dirname, '..', 'docs', 'Manual_Aplicacion_Web.md');
const OUT_PATH = path.join(__dirname, '..', 'docs', 'Manual_Aplicacion_Web.pdf');
...
async function main() {
    ...
    md = md.replace(/```mermaid[\s\S]*?```/g, '*(Ver diagrama interactivo en la versión web de la aplicación)*');
...
${htmlBody}
    ...
    await page.pdf({
        ...
        headerTemplate: '<div style="font-family:Lato,sans-serif;font-size:7pt;color:#999;width:100%;text-align:right;padding-right:18mm;">deCampoacampo — Manual de la Aplicación Web</div>',
    ...
    await browser.close();
    console.log('PDF generado exitosamente:', OUT_PATH);
}
