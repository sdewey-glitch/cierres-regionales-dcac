...
const OUT_PATH = path.join(__dirname, 'docs', 'Cierres_Regionales_deCampoacampo.pdf');
const IMG_DIR = path.join(__dirname, '..', '..', 'brain', '57477c14-f997-40d0-a1db-8688bc845850');

async function main() {
    let md = fs.readFileSync(MD_PATH, 'utf8');
    
    // Convert image paths to base64 data URIs for puppeteer
    // Windows paths contain backslashes, so we need a broader regex
    md = md.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, src) => {
        if (src.startsWith('http') || src.startsWith('data:')) return match;
        const absPath = src.replace(/\\\\/g, '/').replace(/\\/g, '/');
        const resolvedPath = path.isAbsolute(src) ? src : path.resolve(IMG_DIR, src);
        try {
            const imgBuf = fs.readFileSync(resolvedPath);
            const ext = path.extname(resolvedPath).slice(1) || 'png';
            const b64 = imgBuf.toString('base64');
            console.log('Embedded image:', alt, '(' + (imgBuf.length/1024).toFixed(0) + 'KB)');
            return `![${alt}](data:image/${ext};base64,${b64})`;
        } catch (e) {
            console.warn('Image NOT FOUND:', resolvedPath, e.message);
            return `<!-- image not found: ${resolvedPath} -->`;
        }
    });

    // Remove mermaid blocks (replaced by generated images)
...
<div class="cover">
  ...
  <div class="line"></div>
  <h1>Manual de Cierres Regionales</h1>
  <div class="subtitle">Documentación Técnica y Modelo Remunerativo</div>
  <div class="meta">
    <p><strong>Versión 4.0</strong> — Mayo 2026</p>
    <p><strong>Audiencia:</strong> C-Level · Recursos Humanos · Desarrollo Comercial</p>
...
${htmlBody}
    ...
    await page.pdf({
        ...
        headerTemplate: '<div style="font-family:Lato,sans-serif;font-size:7pt;color:#999;width:100%;text-align:right;padding-right:18mm;">deCampoacampo — Cierres Regionales</div>',
        footerTemplate: '<div style="font-family:Lato,sans-serif;font-size:7pt;color:#999;width:100%;text-align:center;"><span class="pageNumber"></span> / <span class="totalPages"></span></div>',
    });
