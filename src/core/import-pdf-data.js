...
const drive_1 = require("../api/drive");
const sheets_1 = require("../api/sheets");
const env_1 = require("../config/env");
const pdf = require('pdf-parse');
const CIERRES_ROOT_FOLDER = env_1.config.CIERRES_ROOT_FOLDER_ID;
const MONTH_MAP = {
...
function parsePdfText(text, filename, folderYear, folderMonth) {
    ...
    if (matchOfi)
        oficina = matchOfi[1].trim();
    let modalidad = '';
    const matchMod = text.match(/M O D A L I D A D\s*\n\s*([^\n]+)/i);
    if (matchMod)
        modalidad = matchMod[1].trim();
    // 11. Ajustes
    let ajustes = 0;
...
async function main() {
    ...
    const yearFolders = await (0, drive_1.listDriveFolders)(CIERRES_ROOT_FOLDER);
    const allResults = [];
    for (const yFolder of yearFolders) {
        ...
        const monthFolders = await (0, drive_1.listDriveFolders)(yFolder.id);
        for (const mFolder of monthFolders) {
            ...
            const files = await (0, drive_1.listDriveFiles)(mFolder.id);
            console.log(`  Encontrados ${files.length} archivos PDF.`);
            for (const file of files) {
                try {
                    console.log(`    Descargando y parseando: "${file.name}"...`);
                    const buffer = await (0, drive_1.downloadDriveFile)(file.id);
    ...
    await (0, sheets_1.createSheetIfNotExists)(env_1.config.TARGET_SPREADSHEET_ID, sheetName);
    await (0, sheets_1.clearSheetRange)(env_1.config.TARGET_SPREADSHEET_ID, `'${sheetName}'!A:P`);
    const headers = [
        [
            "AñoMes",
            "Comercial",
    ...
    await (0, sheets_1.writeSheet)(env_1.config.TARGET_SPREADSHEET_ID, `'${sheetName}'!A1`, headers);
    await (0, sheets_1.appendSheet)(env_1.config.TARGET_SPREADSHEET_ID, `'${sheetName}'!A2`, rows);
