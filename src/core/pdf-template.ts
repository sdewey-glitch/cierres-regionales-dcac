...
export function generateClosureHtml(data: CommercialResult): string {
    ...
    const totalGastosDetalle = (data.gastosDetalle || []).reduce((s: number, g: any) => s + (Number(g.importe) || 0), 0);
    ...
    const operaciones = [...(data.operacionesDetalle || [])].filter((op: any) => !op.excluida).sort((a, b) => 
