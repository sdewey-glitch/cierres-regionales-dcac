...
exports.calculateDynamicMonth = calculateDynamicMonth;
exports.calculateRetroactiveAdjustments = calculateRetroactiveAdjustments;
const metabase_1 = require("../api/metabase");
...
function classifyChannel(acName, repreName, dbChannel, roster) {
    ...
    if (entryToUse && entryToUse.tipo) {
        ...
        if (tipo === 'corporate' || tipo === 'oficina' || tipo === 'operario de carga')
            return 'Directo';
    ...
    if (dbChannel) {
        ...
        if (db.includes('regional'))
            return 'Regional';
        if (db.includes('representante'))
            return 'Representante';
        if (db.includes('comisionista'))
            return 'Comisionista';
        if (db.includes('directo'))
            return 'Directo';
    }
...
let q95Cache = null;
async function calculateDynamicMonth(year, month) {
    // Siempre refrescar datos para evitar caché viciada (importante en Vercel serverless)
    q95Cache = await (0, metabase_1.fetchQ95)();
    ...
    const [kmsData, preciosKm, amortData, mendelData, vehicleMap, gastos, manualAdjustments, tajadaData, historicalSalaries, viajesPropios] = await Promise.all([
        (0, inputs_1.fetchKms)(), (0, inputs_1.fetchPreciosKm)(year, month), (0, inputs_1.fetchAmortDcac)(), (0, inputs_1.fetchMendelGastos)(), (0, inputs_1.fetchVehicleMap)(), (0, inputs_1.fetchGastos)(), (0, inputs_1.fetchAjustesManuales)(), (0, inputs_1.fetchTajada)(), (0, historical_1.fetchHistoricalSalaries)(), (0, inputs_1.fetchViajesPropios)()
    ]);
    ...
    const ops = rawOps.filter(op => {
        ...
        if (invalidStates.includes(estado))
            return false;
        if (invalidStates.includes(estadoTrop))
            return false;
        return true;
    });
    // Cargar Cuentas Especiales
    let cuentasEspeciales = [];
    ...
    if (fs.existsSync(cuentasFile)) {
        try {
            ...
            console.error("Error loading cuentas.json");
        }
    }
    ...
    const getOrCreateResult = (comercial) => {
    ...
    for (const op of ops) {
        ...
        let vendSource = 'none';
        if (acIdVend && acIdVend.trim())
            vendSource = 'legajo';
        else if (acSocVend && acSocVend.trim())
            vendSource = 'sociedad';
        else if (repreVend && repreVend.trim())
            vendSource = 'repre';
        ...
        let compSource = 'none';
        if (acIdComp && acIdComp.trim())
            compSource = 'legajo';
        else if (acSocComp && acSocComp.trim())
            compSource = 'sociedad';
        else if (repreComp && repreComp.trim())
            compSource = 'repre';
        const acVend = await (0, normalization_1.normalizeName)(String(vendRaw));
        const acComp = await (0, normalization_1.normalizeName)(String(compRaw));
        ...
        let marcaVend = '';
        let marcaComp = '';
        if (acVend && vendSource === 'sociedad')
            marcaVend = '⚑'; // AC viene de la sociedad, no del legajo
        if (acComp && compSource === 'sociedad')
            marcaComp = '⚑';
        if (!acVend && !acComp)
            continue; // Sin AC en ningún lado, no se puede asignar
        ...
        if (isVendEmpty && !isCompEmpty) {
            ...
            if (!uniqueSocieties.has(acComp))
                uniqueSocieties.set(acComp, new Set());
            // As comprador, only their side's society counts
            if (op.RS_Compradora || op.sociedad_compradora || op.comprador_nombre)
                uniqueSocieties.get(acComp).add(String(op.RS_Compradora || op.sociedad_compradora || op.comprador_nombre).toUpperCase().trim());
            ...
            if (isMag) {
                resComp.cabMag += cabezas;
                resComp.resMag += resultadoFinalAjustado * (1 / 3);
            }
            else if (isFaenaCat) {
                resComp.cabFaena += cabezas;
                resComp.resFaena += resultadoFinalAjustado * (1 / 3);
            }
            else if (isCria) {
                resComp.cabCria += cabezas;
                resComp.resCria += resultadoFinalAjustado * (1 / 3);
            }
            else if (isInvNeo) {
                resComp.cabInvNeo += cabezas;
                resComp.resInvNeo += resultadoFinalAjustado * (1 / 3);
            }
            else {
                resComp.cabInv += cabezas;
                resComp.resInv += resultadoFinalAjustado * (1 / 3);
            }
            ...
            if (!uniqueSocieties.has(acVend))
                uniqueSocieties.set(acVend, new Set());
            // As vendedor, only their side's society counts
            if (op.RS_Vendedora || op.sociedad_vendedora || op.vendedor_nombre)
                uniqueSocieties.get(acVend).add(String(op.RS_Vendedora || op.sociedad_vendedora || op.vendedor_nombre).toUpperCase().trim());
            ...
            const isInvNeo = (op.UN || '').toLowerCase().includes('neo');
            if (isMag) {
                resVend.cabMag += cabezas;
                resVend.resMag += resultadoFinalAjustado * (2 / 3);
            }
            else if (isFaenaCat) {
                resVend.cabFaena += cabezas;
                resVend.resFaena += resultadoFinalAjustado * (2 / 3);
            }
            else if (isCria) {
                resVend.cabCria += cabezas;
                resVend.resCria += resultadoFinalAjustado * (2 / 3);
            }
            else if (isInvNeo) {
                resVend.cabInvNeo += cabezas;
                resVend.resInvNeo += resultadoFinalAjustado * (2 / 3);
            }
            else {
                resVend.cabInv += cabezas;
                resVend.resInv += resultadoFinalAjustado * (2 / 3);
            }
    ...
    const createPoolStats = () => ({
        cabezas: 0, resultado: 0, socOpGen: 0, count: 0, tropas: 0, loteIds: new Set(),
        resInv: 0, resFaena: 0, resNeo: 0, resCria: 0, resMag: 0,
        cabInv: 0, cabFaena: 0, cabNeo: 0, cabCria: 0, cabMag: 0
    });
    const officeAgentPools = new Map();
    const officePseudoAgentPools = new Map();
    for (const res of resultsMap.values()) {
        ...
        for (const det of res.operacionesDetalle) {
            if (!currentPool.loteIds.has(det.id_lote)) {
                ...
                if (unLower.includes('neo') || tipoLower.includes('neo'))
                    currentPool.cabNeo += cant;
                else if (tipoLower.includes('invernada'))
                    currentPool.cabInv += cant;
                else if (tipoLower.includes('faena'))
                    currentPool.cabFaena += cant;
                else if (tipoLower.includes('cria') || tipoLower.includes('cría'))
                    currentPool.cabCria += cant;
                else
                    currentPool.cabMag += cant;
            }
        }
            ...
            for (const det of res.operacionesDetalle) {
                if (!current.loteIds.has(det.id_lote)) {
                    ...
                    if (unLower.includes('neo') || tipoLower.includes('neo'))
                        ...
                        current.cabFaena += cant;
                    else if (tipoLower.includes('cria') || tipoLower.includes('cría'))
                        current.cabCria += cant;
                    else
                        current.cabMag += cant;
                }
            }
    ...
    for (const res of finalResults) {
        ...
        const baseMinimo = await (0, calculator_1.getMinimumForCategory)(Number(res.categoria) || 0, year, month);
        ...
        const configModel = isStdOrSpecial ? { id: res.modalidad.toLowerCase() } : await (0, models_1.getModelByModalidad)(res.modalidad);
        ...
        if (isCustomModel) {
            ...
            if (configModel.componenteP?.activa) {
                ...
                if (res.cabezasGeneral >= umbral) {
                    pctPersonal = await (0, models_1.resolveScalePct)(res.cabezasGeneral, configModel.componenteP.escalaId || 'escalaAC', year, month);
                }
                else {
                    pctPersonal = 0;
                }
            ...
            if (isOperario || isFijo) {
                ...
                pctPersonal = await (0, calculator_1.getExactScale)(res.cabezasGeneral, 'escalaPersonal', year, month);
            }
            else {
                pctPersonal = await (0, calculator_1.getExactScale)(res.cabezasGeneral, 'escalaAC', year, month);
            }
            if (isAcuña) {
                let totalAcuna = 0;
                for (const det of res.operacionesDetalle) {
                    ...
                    const cuentaMatch = cuentasAcuna.find(c => (isVenta && c.razon_social && c.razon_social.toLowerCase() === det.sociedad_vendedora?.toLowerCase()) ||
                ...
                const cuentaVigente = (cuenta) => {
                    if (!cuenta || !cuenta.fecha_incorporacion)
                        return true; // sin fecha = vigente por defecto
                ...
                for (const det of res.operacionesDetalle) {
                    ...
                    if (esCtaEspecial) {
                        ...
                        if (cuentaMatchVenta && cuentaMatchVenta.tipo_cuenta === 'Mermas' && cuentaVigente(cuentaMatchVenta)) {
                            if (det.tipo.toLowerCase().includes('faena'))
                                pctVenta = 0.20;
                            else
                                pctVenta = 0.15;
                            catVenta = 'Mermas';
                    ...
                    if (catVenta === 'Mermas')
                        mermasTotal += det.ganancia_personal_venta;
                    else if (catVenta === 'Operaciones Grandes')
                        gcTotal += det.ganancia_personal_venta;
                    if (catCompra === 'Activacion CI')
                        cisTotal += det.ganancia_personal_compra;
                    else if (catCompra === 'Operaciones Grandes')
                        gcTotal += det.ganancia_personal_compra;
                }
        ...
        res.resInvReg = agentPool.resInv;
        res.cabInvReg = agentPool.cabInv;
        res.resFaenaReg = agentPool.resFaena;
        res.cabFaenaReg = agentPool.cabFaena;
        res.resCriaReg = agentPool.resCria;
        res.cabCriaReg = agentPool.cabCria;
        res.resMagReg = agentPool.resMag;
        res.cabMagReg = agentPool.cabMag;
        res.resInvNeoReg = agentPool.resNeo;
        res.cabInvNeoReg = agentPool.cabNeo;
        // V3.0: Bolsa se divide por 2 para Buenos Aires
        // V3.0: Bolsa se divide por 2 para Buenos Aires
        let bolsaScale = await (0, calculator_1.getExactScale)(res.cabezasRegional, 'escalaProvincial', year, month);
        ...
        res.resInvOfi = pseudoPool.resInv;
        res.cabInvOfi = pseudoPool.cabInv;
        res.resFaenaOfi = pseudoPool.resFaena;
        res.cabFaenaOfi = pseudoPool.cabFaena;
        res.resCriaOfi = pseudoPool.resCria;
        res.cabCriaOfi = pseudoPool.cabCria;
        res.resMagOfi = pseudoPool.resMag;
        res.cabMagOfi = pseudoPool.cabMag;
        res.resInvNeoOfi = pseudoPool.resNeo;
        res.cabInvNeoOfi = pseudoPool.cabNeo;
        res.escalaOficina = await (0, calculator_1.getExactScale)(res.cabezasOfi, 'escalaOficina', year, month);
        ...
        if (kmsEntry) {
            ...
            if (!resolvedType && amortEntry) {
                ...
                if (model.includes('hilux') || model.includes('ranger') || model.includes('amarok'))
                    resolvedType = 'camioneta';
                else if (model.includes('sw4') || model.includes('fortuner') || model.includes('tracker'))
                    resolvedType = 'suv';
                else
                    resolvedType = 'auto';
            }
        ...
        const normalizeName = (n) => (n || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
        ...
        const manualSacAdj = res.retroactivosDetalle?.find(adj => adj.motivo?.toLowerCase().includes('aguinaldo') ||
            adj.motivo?.toLowerCase().includes('sac') ||
            adj.observaciones?.toLowerCase().includes('aguinaldo'));
        if (manualSacAdj) {
            ...
            if (!isAcunaOrFrutos && !exentosAguinaldo.includes(res.asociadoComercial.toLowerCase())) {
                ...
                for (let i = 1; i <= 5; i++) {
                    ...
                    const prevBaseMinimo = await (0, calculator_1.getMinimumForCategory)(Number(res.categoria) || 0, y, m);
...
async function calculateRetroactiveAdjustments(year, month) {
    const adjustments = [];
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        const frozenData = await (0, snapshot_1.loadMonthSnapshot)(pastYear, pastMonth);
        if (!frozenData) {
