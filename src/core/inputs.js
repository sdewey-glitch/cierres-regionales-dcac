...
exports.cleanSheetsNumber = cleanSheetsNumber;
exports.fetchGastos = fetchGastos;
exports.fetchAjustesManuales = fetchAjustesManuales;
exports.fetchTajada = fetchTajada;
exports.fetchKms = fetchKms;
exports.fetchPreciosKm = fetchPreciosKm;
exports.fetchVehicleMap = fetchVehicleMap;
exports.fetchAmortDcac = fetchAmortDcac;
exports.fetchMendelGastos = fetchMendelGastos;
exports.fetchViajesPropios = fetchViajesPropios;
const sheets_1 = require("../api/sheets");
const env_1 = require("../config/env");
const normalization_1 = require("./normalization");
function cleanSheetsNumber(val) {
...
const CONFIG_ID = env_1.config.HUB_CONFIGURACIONES_ID;
const GASTOS_ID = env_1.config.HUB_GASTOS_ID;
const CIERRES_ID = env_1.config.HUB_CIERRES_ID;
const KMS_VIAJES_ID = env_1.config.KMS_VIAJES_ID;
const TABLERO_ID = env_1.config.HUB_TABLERO_ID;
const HISTORIAL_ID = env_1.config.HUB_HISTORIAL_ID;
async function fetchGastos() {
    try {
        const data = await (0, sheets_1.readSheet)(GASTOS_ID, "'BDGASTOS'!A2:E");
...
async function fetchAjustesManuales() {
    try {
        // Nuevo formato: Año(A) | Mes(B) | AñoMes(C) | Asociado_Comercial(D) | Motivo(E) | Monto(F)
        const data = await (0, sheets_1.readSheet)(CONFIG_ID, "'Ajustes'!A2:F");
...
async function fetchKms() {
    try {
        ...
        const data = await (0, sheets_1.readSheet)(GASTOS_ID, "'KMS'!A2:H");
        return data.filter(row => row[0]).map(row => ({
            año: Number(row[0]),
            mes: Number(row[1]),
...
function normalizeTipoVehiculo(tipo) {
...
async function fetchPreciosKm(year, month) {
    const KMS_STOCK_ID = env_1.config.KMS_STOCK_ID;
    // ── Fuente nueva: Stock Autos - Kms → pestaña $xKMs ──
    // Formato: Año | Mes | $ x km | Tipo (suv/chata/auto)
    if (KMS_STOCK_ID && year && month) {
        try {
            const data = await (0, sheets_1.readSheet)(KMS_STOCK_ID, "'$xKMs'!A2:D");
            const prices = new Map();
            // Buscar fila exacta para el año/mes pedido
            const exactRows = data.filter(row => Number(row[0]) === year && Number(row[1]) === month && row[2] && row[3]);
            ...
            if (exactRows.length === 0) {
                ...
                if (candidates.length > 0) {
                    ...
                    rowsToUse = candidates.filter(row => Number(row[0]) === latestYear && Number(row[1]) === latestMonth);
    ...
    try {
        const data = await (0, sheets_1.readSheet)(GASTOS_ID, "'Kms & $'!A2:Z10");
        const prices = new Map();
...
async function fetchVehicleMap() {
    const KMS_STOCK_ID = env_1.config.KMS_STOCK_ID;
    const map = new Map();
    ...
    if (KMS_STOCK_ID) {
        try {
            const data = await (0, sheets_1.readSheet)(KMS_STOCK_ID, "'Comerciales'!A2:F");
            for (const row of data) {
    ...
    try {
        const data = await (0, sheets_1.readSheet)(GASTOS_ID, "'Kms & $'!A6:B30");
        for (const row of data) {
...
async function fetchAmortDcac() {
    try {
        // Amort_DCAC: Asociado Comercial | Provincia | Año | Modelo | 2024 | 2025
        const data = await (0, sheets_1.readSheet)(GASTOS_ID, "'Amort_DCAC'!A2:F");
        return data.filter(row => row[0]).map(row => ({
...
async function fetchMendelGastos() {
    try {
        const spreadsheetId = env_1.config.MENDEL_SPREADSHEET_ID;
        // 1. Read Base Mendel
        const rawMendel = await (0, sheets_1.readSheet)(spreadsheetId, "'Base Mendel'!A2:BC");
        // 2. Read Copia de Correlaciones usuarios Mendel
        const rawCorr = await (0, sheets_1.readSheet)(spreadsheetId, "'Correlaciones usuarios Mendel'!A2:B");
        // 3. Get Roster
 ...
 */
async function fetchViajesPropios() {
    try {
        if (!KMS_VIAJES_ID) {
        ...
        const raw = await (0, sheets_1.readSheet)(KMS_VIAJES_ID, "'Viajes CRM - AutosPropios'!A2:J");
        ...
        for (const row of raw) {
            ...
            if (!fechaSerial || fechaSerial < 45000 || km <= 0 || km > 50000)
                continue;
