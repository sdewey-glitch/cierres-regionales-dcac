...
let minimosCache: MinimoRow[] | null = null;
let escalasCache: EscalaRow[] | null = null;
let lastFetchTime = 0;
const CACHE_TTL_MS = 60000; // 60 seconds

export function interpolateLogScale(value: number, minBound: number, maxBound: number, minPct: number, maxPct: number): number {
    if (value <= minBound) return maxPct / 100;
    if (value >= maxBound) return minPct / 100;
...
export async function loadConfig(): Promise<void> {
    const now = Date.now();
    if (minimosCache && escalasCache && (now - lastFetchTime < CACHE_TTL_MS)) return;

    try {
        const minimosData = await readSheet(config.HUB_SPREADSHEET_ID, 'Config_Minimos!A2:G');
        ...
        const escalasData = await readSheet(config.HUB_SPREADSHEET_ID, 'Config_Escalas!A2:G');
        ...
        if (!minimosCache) minimosCache = [];
        if (!escalasCache) escalasCache = [];
    }
}

function getClosestPeriodConfig<T extends { anoMes: number }>(cache: T[], targetPeriodo: number): T[] {
...
export async function getMinimumForCategory(category: number, year: number, month: number): Promise<number> {
    await loadConfig();
...
export async function getExactScale(cabezasRaw: number, type: 'escalaPersonal' | 'escalaProvincial' | 'escalaOficina' | 'escalaAC', year: number, month: number): Promise<number> {
    await loadConfig();
    const targetPeriodo = Number(`${year}${month.toString().padStart(2, '0')}`);
    ...
    if (resultPct > maxScale) resultPct = maxScale;
    if (resultPct < minScale) resultPct = minScale;
