...
import { CommercialResult, OperacionDetalle } from './types';
...
// ── Inicializar hojas ────────────────────────────────────────────────────────
export async function initHistoricSheets(): Promise<void> {
    await createSheetIfNotExists(config.HUB_CIERRES_ID, SHEET_CIERRES);
    await createSheetIfNotExists(config.HUB_CIERRES_ID, SHEET_TROPAS);

    // Escribir headers si la hoja está vacía
    const existing = await readSheet(config.HUB_CIERRES_ID, `'${SHEET_CIERRES}'!A1:A1`).catch(() => []);
    if (!existing || existing.length === 0 || !existing[0] || existing[0][0] !== 'Periodo') {
        await writeSheet(config.HUB_CIERRES_ID, `'${SHEET_CIERRES}'!A1`, [HEADERS_CIERRES]);
    }

    const existingT = await readSheet(config.HUB_CIERRES_ID, `'${SHEET_TROPAS}'!A1:A1`).catch(() => []);
    if (!existingT || existingT.length === 0 || !existingT[0] || existingT[0][0] !== 'Periodo_Cierre') {
        await writeSheet(config.HUB_CIERRES_ID, `'${SHEET_TROPAS}'!A1`, [HEADERS_TROPAS]);
    }
}

// ── Guardar cierre al congelar ───────────────────────────────────────────────
export async function saveHistoricCierre(
    year: number,
    month: number,
    agentData: CommercialResult
): Promise<void> {
    ...
    // Filas de tropas
    const tropaRows: any[][] = (agentData.operacionesDetalle || []).map(op => {
        const opYM = op.fecha_operacion
            ? op.fecha_operacion.substring(0, 4) + op.fecha_operacion.substring(5, 7)
            : periodo;
        const ganancia =
            (op.comercial_venta?.toLowerCase() === agentData.asociadoComercial.toLowerCase() ? (op.ganancia_personal_venta || 0) : 0) +
    ...
    const existingCierres = await readSheet(config.HUB_CIERRES_ID, RANGE_CIERRES).catch(() => []);
    const existingIdx = (existingCierres || []).findIndex((r, i) =>
        i > 0 &&
        String(r[0]) === periodo &&
    ...
    if (existingIdx > 0) {
        ...
        await writeSheet(config.HUB_CIERRES_ID, `'${SHEET_CIERRES}'!A${sheetRow}`, [cierreRow]);
        console.log(`[historico] 🔄 Actualizado cierre de ${agentData.asociadoComercial} ${periodo} (fila ${sheetRow})`);
    } else {
        await appendSheet(config.HUB_CIERRES_ID, RANGE_CIERRES, [cierreRow]);
    }

    // Tropas: eliminar las anteriores del mismo agente/período y reescribir
    if (tropaRows.length > 0) {
        const existingTropas = await readSheet(config.HUB_CIERRES_ID, RANGE_TROPAS).catch(() => []);
        if (existingTropas && existingTropas.length > 1) {
            const headers = existingTropas[0];
            const otherTropas = existingTropas.slice(1).filter(r =>
                !(String(r[0]) === periodo && String(r[3]).toLowerCase() === agentData.asociadoComercial.toLowerCase())
            );
            await clearSheetRange(config.HUB_CIERRES_ID, `'${SHEET_TROPAS}'!A2:T100000`).catch(() => {});
            await writeSheet(config.HUB_CIERRES_ID, `'${SHEET_TROPAS}'!A1`, [headers, ...otherTropas, ...tropaRows]);
        } else {
            await appendSheet(config.HUB_CIERRES_ID, RANGE_TROPAS, tropaRows);
        }
    }
...
export async function loadHistoricCierre(
    year: number,
    month: number,
    agentName: string
    ...
    const [cierresData, tropasData] = await Promise.all([
        readSheet(config.HUB_CIERRES_ID, RANGE_CIERRES).catch(() => []),
        readSheet(config.HUB_CIERRES_ID, RANGE_TROPAS).catch(() => []),
    ]);

    if (!cierresData || cierresData.length < 2) return null;

    // Buscar fila del cierre (skip header row)
    const row = cierresData.slice(1).find(r =>
        String(r[0]) === periodo &&
    ...
    let gastosDetalle: { categoria: string; importe: number }[] = [];
    try { gastosDetalle = JSON.parse(String(row[33] || '[]')); } catch { gastosDetalle = []; }

    // Parsear tropas
    const tropas: OperacionDetalle[] = (tropasData || []).slice(1)
        .filter(r =>
            String(r[0]) === periodo &&
    ...
    const todasTropas: OperacionDetalle[] = (tropasData || []).slice(1)
        .filter(r =>
            String(r[0]) === periodo &&
            String(r[3]).toLowerCase() === agentName.toLowerCase()
        )
...
export async function deleteHistoricCierre(
    year: number,
    month: number,
    agentName: string
): Promise<void> {
    ...
    const cierresData = await readSheet(config.HUB_CIERRES_ID, RANGE_CIERRES).catch(() => []);
    if (cierresData && cierresData.length > 1) {
        const headers = cierresData[0];
        const remaining = cierresData.slice(1).filter(r =>
            !(String(r[0]) === periodo && String(r[1]).toLowerCase() === agentName.toLowerCase())
        );
        const colCount = HEADERS_CIERRES.length;
        const endCol = String.fromCharCode(65 + colCount - 1); // A=65
        await clearSheetRange(config.HUB_CIERRES_ID, `'${SHEET_CIERRES}'!A2:${endCol}100000`).catch(() => {});
        if (remaining.length > 0) {
            await writeSheet(config.HUB_CIERRES_ID, `'${SHEET_CIERRES}'!A1`, [headers, ...remaining]);
        } else {
            await writeSheet(config.HUB_CIERRES_ID, `'${SHEET_CIERRES}'!A1`, [headers]);
        }
    }

    // Limpiar Historico_Tropas
    const tropasData = await readSheet(config.HUB_CIERRES_ID, RANGE_TROPAS).catch(() => []);
    if (tropasData && tropasData.length > 1) {
        const headers = tropasData[0];
        const remaining = tropasData.slice(1).filter(r =>
            !(String(r[0]) === periodo && String(r[3]).toLowerCase() === agentName.toLowerCase())
        );
        await clearSheetRange(config.HUB_CIERRES_ID, `'${SHEET_TROPAS}'!A2:T100000`).catch(() => {});
        if (remaining.length > 0) {
            await writeSheet(config.HUB_CIERRES_ID, `'${SHEET_TROPAS}'!A1`, [headers, ...remaining]);
        } else {
            await writeSheet(config.HUB_CIERRES_ID, `'${SHEET_TROPAS}'!A1`, [headers]);
        }
    }
...
export async function migrateExistingFrozenCierres(
    frozenList: { period: string; agentName: string }[],
    loadSnapshot: (year: number, month: number) => Promise<any[] | null>
): Promise<{ migrated: string[], skipped: string[], errors: string[] }> {
    // Leer qué períodos+agentes ya están en Historico_Cierres
    const existing = await readSheet(config.HUB_CIERRES_ID, `'${SHEET_CIERRES}'!A:B`).catch(() => []);
    const existingSet = new Set(
        (existing || []).slice(1).map(r => `${String(r[0])}_${String(r[1]).toLowerCase()}`)
    );
    ...
    for (const { period, agentName } of frozenList) {
        ...
        const agentData = snap.find((r: any) =>
            r.asociadoComercial?.toLowerCase() === agentName.toLowerCase()
        );
        if (!agentData) {
        ...
        try {
            await saveHistoricCierre(year, month, agentData as CommercialResult);
