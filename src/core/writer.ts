import { readSheet, writeSheet, clearSheetRange } from '../api/sheets';
...
export async function updateDynamicSueldos(year: number, month: number, results: CommercialResult[]): Promise<void> {
    ...
    try {
        existingRows = await readSheet(spreadsheetId, `'${sheetName}'!A2:AZ20000`);
    } catch (err: any) {
    ...
    const otherMonthsRows = existingRows.filter(row => {
        if (row.length < 4) return true; // Preservar filas extrañas/vacías
    ...
    await clearSheetRange(spreadsheetId, `'${sheetName}'!A1:AZ20000`);

    // 5. Escribir
    await writeSheet(spreadsheetId, `'${sheetName}'!A1:AZ${allData.length}`, allData);
