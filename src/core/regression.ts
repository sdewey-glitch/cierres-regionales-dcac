export interface RegressionInput {
    ...
    r: number;
    o: number;
    target: number; // Sueldo real pagado (ej: sueldo bruto o sueldo neto)
}
...
export interface RegressionResult {
    ...
    coefO: number;
    intercepto: number;
    r2: number; // Coeficiente de determinación R^2 (0 a 1)
    rmse: number; // Error cuadrático medio raíz (en pesos)
}
 ...
 */
export function runRegression(data: RegressionInput[]): RegressionResult {
    const N = data.length;
    if (N < 4) {
