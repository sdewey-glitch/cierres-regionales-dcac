...
let lastV3Fetch = 0;
app.get('/api/v3-salaries', async (req, res) => {
    try {
        const now = Date.now();
...
app.get('/api/retroactivos', async (req, res) => {
...
app.get('/api/kms-prices', async (req, res) => {
    try {
...
app.get('/api/ajustes-manuales', async (req, res) => {
    try {
...
app.post('/api/ajustes-manuales', async (req, res) => {
    try {
...
app.put('/api/ajustes-manuales/:id', async (req, res) => {
    try {
...
app.delete('/api/ajustes-manuales/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id) || id < 0) {
            return res.status(400).json({ error: 'ID inválido' });
...
app.get('/api/revision/sociedad-sin-legajo', async (req, res) => {
    try {
...
app.get('/api/revision/concretadas-sin-cierre', async (req, res) => {
    try {
        const year = parseInt(String(req.query.year), 10);
        const month = parseInt(String(req.query.month), 10);
        if (!year || !month) return res.status(400).json({ error: 'year y month requeridos' });
...
import { getCierreLinks } from './api/drive';
...
app.get('/api/metricas-red', async (req, res) => {
    try {
        const snapshotDir = path.join(__dirname, 'core/snapshots');
        const MONTHS_ES = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
