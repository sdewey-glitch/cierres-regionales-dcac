...
export async function formatSheetNumbers(spreadsheetId: string, sheetName: string, startColIndex: number, endColIndex: number, customPattern?: string) {
    const sheets = getSheetsClient();
            ...
            requests: [
                {
                    repeatCell: {
                        ...
                        cell: {
                            userEnteredFormat: {
                                numberFormat: {
                                    type: customPattern ? 'NUMBER' : 'CURRENCY',
                                    pattern: customPattern || '"$"#,##0.00'
                                }
                            }
                        },
...
export async function formatIdentityColumns(spreadsheetId: string, sheetName: string) {
    const sheets = getSheetsClient();
    const metadata = await sheets.spreadsheets.get({ spreadsheetId });
    const sheet = metadata.data.sheets?.find(s => s.properties?.title === sheetName);
    if (!sheet || !sheet.properties?.sheetId) return;
...
export async function formatSheetIntegers(spreadsheetId: string, sheetName: string, startColIndex: number, endColIndex: number) {
    const sheets = getSheetsClient();
    const metadata = await sheets.spreadsheets.get({ spreadsheetId });
    const sheet = metadata.data.sheets?.find(s => s.properties?.title === sheetName);
    if (!sheet || !sheet.properties?.sheetId) return;
