...
exports.getMetabaseSession = getMetabaseSession;
exports.fetchCard = fetchCard;
exports.fetchQ95 = fetchQ95;
exports.fetchAcAssignmentDates = fetchAcAssignmentDates;
...
async function getMetabaseSession() {
    if (cachedToken && Date.now() < tokenExpiry)
        return cachedToken;
    const baseUrl = env_1.config.METABASE_URL.replace(/\/$/, ''); // Remove trailing slash
    console.log(`Authenticating Metabase at ${baseUrl}/api/session...`);
    const res = await fetch(`${baseUrl}/api/session`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            username: env_1.config.METABASE_USERNAME,
            password: env_1.config.METABASE_PASSWORD
        }),
    });
    if (!res.ok) {
...
async function fetchCard(cardId, sessionToken, parameters) {
    const baseUrl = env_1.config.METABASE_URL.replace(/\/$/, '');
 ...
 */
async function fetchAcAssignmentDates() {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(`Failed to fetch AC assignment dates: ${res.status}`);
        const data = await res.json();
        const mapping = {};
