...
export async function loadAgentDataFromBajada(
    ...
    if (provinciaNombre) {
        ...
        for (const [, lote] of bajadaMap) {
            ...
            if (pVend !== provinciaNombre && pComp !== provinciaNombre) continue;
            ...
            if      (tipoLow.includes('neo'))                                 { regResNeo   += lote.resultadoTopeado; regCabNeo   += lote.cantidad; }
            else if (tipoLow.includes('invernada'))                           { regResInv   += lote.resultadoTopeado; regCabInv   += lote.cantidad; }
            else if (tipoLow.includes('faena'))                               { regResFaena += lote.resultadoTopeado; regCabFaena += lote.cantidad; }
            else if (tipoLow.includes('cria') || tipoLow.includes('cría'))   { regResCria  += lote.resultadoTopeado; regCabCria  += lote.cantidad; }
            else                                                              { regResMag   += lote.resultadoTopeado; regCabMag   += lote.cantidad; }
        }
    ...
    if (oficinaNombre) {
        ...
        for (const [, lote] of bajadaMap) {
            ...
            const tipoLow = (lote.tipo || '').toLowerCase();
            if      (tipoLow.includes('neo'))                                 { ofiResNeo   += lote.resultadoTopeado; ofiCabNeo   += lote.cantidad; }
            else if (tipoLow.includes('invernada'))                           { ofiResInv   += lote.resultadoTopeado; ofiCabInv   += lote.cantidad; }
            else if (tipoLow.includes('faena'))                               { ofiResFaena += lote.resultadoTopeado; ofiCabFaena += lote.cantidad; }
            else if (tipoLow.includes('cria') || tipoLow.includes('cría'))   { ofiResCria  += lote.resultadoTopeado; ofiCabCria  += lote.cantidad; }
            else                                                              { ofiResMag   += lote.resultadoTopeado; ofiCabMag   += lote.cantidad; }
        }
