...
import { fetchTajada } from '../core/inputs';
...
// ── Helpers de columna ───────────────────────────────────────────────────────
const col = (row: any[], idx: number): string => String(row[idx] ?? '').trim();
const num = (row: any[], idx: number): number => {
    const raw = row[idx];
...
// ── Leer y agrupar bajada por id_lote ────────────────────────────────────────
interface BajadaLote {
    id_lote: number;
    tipo: string;
    cantidad: number;
    fecha: string;
    añoMes: string;
    rsVendedora: string;   // idx 2  = C = sociedad_vendedora (para tajada = soc. únicas)
    rsCompradora: string;  // idx 3  = D = sociedad_compradora (para tajada = soc. únicas)
    acVendedor: string;
    acComprador: string;
    provACVend: string;    // idx 26 = AA = Prov_AC_Vendedor (para Componente Regional)
    provACComp: string;    // idx 27 = AB = Prov_AC_Comprador (para Componente Regional)
    ofVendedora: string;   // idx 31 = AF = Ofi_Vendedora   (para Componente Oficina)
    ofCompradora: string;  // idx 32 = AG = Ofi_Compradora  (para Componente Oficina)
    resultadoTopeado: number;   // resultado del LOTE completo
    filas: number;              // cuántas filas hay para este lote
}

export async function readBajada(sheetName = BAJADA_SHEET, filterYear?: number, filterMonth?: number): Promise<Map<number, BajadaLote>> {
    const rows = await readSheet(config.HUB_CIERRES_ID, `'${sheetName}'!A:AU`);
    if (!rows || rows.length < 2) throw new Error(`Hoja "${sheetName}" vacía o no encontrada`);

    const map = new Map<number, BajadaLote>();

    for (let i = 1; i < rows.length; i++) {
        const r = rows[i];
        const idLote = parseInt(col(r, 0));
        if (!idLote || isNaN(idLote)) continue;
        ...
        if (filterYear !== undefined && filterMonth !== undefined) {
            const fechaRaw = r[33];
            let rowYear: number | undefined, rowMonth: number | undefined;
        ...
        if (existing) {
            existing.filas++;
            if (col(r, 2)  && !existing.rsVendedora)  existing.rsVendedora  = col(r, 2);
            if (col(r, 3)  && !existing.rsCompradora) existing.rsCompradora = col(r, 3);
            if (col(r, 24) && !existing.acVendedor)   existing.acVendedor   = col(r, 24);
            if (col(r, 25) && !existing.acComprador)  existing.acComprador  = col(r, 25);
            if (col(r, 26) && !existing.provACVend)   existing.provACVend   = col(r, 26);
            if (col(r, 27) && !existing.provACComp)   existing.provACComp   = col(r, 27);
            if (col(r, 31) && !existing.ofVendedora)  existing.ofVendedora  = col(r, 31);
            if (col(r, 32) && !existing.ofCompradora) existing.ofCompradora = col(r, 32);
        } else {
            map.set(idLote, {
                id_lote: idLote,
                tipo:    col(r, 21),
...
// el envío del mes.
export async function loadAgentDataFromBajada(
    year: number,
    month: number,
    agentName: string,
    sheetName = BAJADA_SHEET
): Promise<CommercialResult | null> {
    const [bajadaMap, snapshot] = await Promise.all([
        readBajada(sheetName, year, month), // solo lotes del período por fecha de operación
        loadMonthSnapshot(year, month),
    ]);
    console.log(`[bajada] 📌 Mapa bajada cargado: ${bajadaMap.size} lotes para ${year}/${month}`);

    if (!snapshot) return null;

    const agentData: CommercialResult | undefined = (snapshot as any[]).find(
        (d: any) => d.asociadoComercial?.toString().trim().toLowerCase() === agentName.trim().toLowerCase()
    );
    if (!agentData) return null;

    // Deep clone para no mutar el snapshot
    const result: CommercialResult = JSON.parse(JSON.stringify(agentData));

    const escala: number = result.escalaGen || 0;
    let resultadoFinalBajada = 0;
    let cabezasBajada = 0;
    let tropasBajadaSet = new Set<number>();

    // Parchear cada tropa con el resultado de la bajada
    for (const op of (result.operacionesDetalle || []) as any[]) {
    ...
    result.operacionesDetalle = (result.operacionesDetalle || []).filter(
        (op: any) => !op._excludedByBajada
    );
    ...
    if (esOperario || esFijo) {
        ...
        nuevaEscala = await getExactScale(result.cabezasGeneral, 'escalaPersonal', year, month);
    } else {
        nuevaEscala = await getExactScale(result.cabezasGeneral, 'escalaAC', year, month);
    }
    ...
    if (provinciaNombre) {
        ...
        for (const [, lote] of bajadaMap) {
            ...
            if      (tipoLow.includes('neo'))                                 { regResNeo   += resProporcional; regCabNeo   += lote.cantidad; }
            else if (tipoLow.includes('invernada'))                           { regResInv   += resProporcional; regCabInv   += lote.cantidad; }
            else if (tipoLow.includes('faena'))                               { regResFaena += resProporcional; regCabFaena += lote.cantidad; }
            else if (tipoLow.includes('cria') || tipoLow.includes('cría'))   { regResCria  += resProporcional; regCabCria  += lote.cantidad; }
            else                                                              { regResMag   += resProporcional; regCabMag   += lote.cantidad; }
        }

        result.tropasRegional  = regTropas;
        result.cabezasRegional = regCabezas;
        result.resultadoReg    = regResultado;
        result.resInvReg    = regResInv;  result.cabInvReg    = regCabInv;
        result.resFaenaReg  = regResFaena; result.cabFaenaReg  = regCabFaena;
        result.resCriaReg   = regResCria; result.cabCriaReg   = regCabCria;
        result.resMagReg    = regResMag;  result.cabMagReg    = regCabMag;
        result.resInvNeoReg = regResNeo;  result.cabInvNeoReg = regCabNeo;

        const bolsaScale = await getExactScale(regCabezas, 'escalaProvincial', year, month);
        ...
        const acUniqueSoc = new Map<string, Set<string>>();
        for (const [, lote] of bajadaMap) {
            if (lote.acVendedor) {
                const ac = lote.acVendedor.trim().toLowerCase();
                if (poolACs.has(ac) && lote.rsVendedora) {
                    if (!acUniqueSoc.has(ac)) acUniqueSoc.set(ac, new Set<string>());
        ...
        let totalPoolSoc = 0;
        for (const [, socSet] of acUniqueSoc) totalPoolSoc += socSet.size;
        ...
        try {
            const tajadaData = await fetchTajada();
            const tajadaHist = tajadaData.find(
                (t: any) => String(t.añoMes) === añoMesPad
    ...
    if (oficinaNombre) {
        ...
        for (const [, lote] of bajadaMap) {
            ...
            if (ofVend !== oficinaNombre && ofComp !== oficinaNombre) continue;
            ...
            if (hasRealACVend || hasRealACComp) continue;
            ...
            const tipoLow = (lote.tipo || '').toLowerCase();
            if      (tipoLow.includes('neo'))                                 { ofiResNeo   += resProporcional; ofiCabNeo   += lote.cantidad; }
            else if (tipoLow.includes('invernada'))                           { ofiResInv   += resProporcional; ofiCabInv   += lote.cantidad; }
            else if (tipoLow.includes('faena'))                               { ofiResFaena += resProporcional; ofiCabFaena += lote.cantidad; }
            else if (tipoLow.includes('cria') || tipoLow.includes('cría'))   { ofiResCria  += resProporcional; ofiCabCria  += lote.cantidad; }
            else                                                              { ofiResMag   += resProporcional; ofiCabMag   += lote.cantidad; }
        }

        result.tropasOficina  = ofiTropas;
        result.cabezasOfi     = ofiCabezas;
        result.resultadoOfi   = ofiResultado;
        result.resInvOfi    = ofiResInv;  result.cabInvOfi    = ofiCabInv;
        result.resFaenaOfi  = ofiResFaena; result.cabFaenaOfi  = ofiCabFaena;
        result.resCriaOfi   = ofiResCria; result.cabCriaOfi   = ofiCabCria;
        result.resMagOfi    = ofiResMag;  result.cabMagOfi    = ofiCabMag;
        result.resInvNeoOfi = ofiResNeo;  result.cabInvNeoOfi = ofiCabNeo;

        const escalaOfi = await getExactScale(ofiCabezas, 'escalaOficina', year, month);
    ...
    if (reintegroNeto > 0) reintegroNeto -= (result.gastosMendelMovilidad || 0);
...
router.get('/bajada/debug-lote/:id', async (req, res) => {
    try {
        ...
        const rows = await readSheet(config.HUB_CIERRES_ID, `'Bajada'!A:AU`);
        if (!rows || rows.length < 2) return res.json({ error: 'Hoja vacía' });

        const header = rows[0] || [];
        const found = rows.filter((r, i) => i > 0 && Number(r[0]) === idBuscado);

        if (found.length === 0) return res.json({ error: `Lote ${idBuscado} no encontrado` });
...
router.get('/bajada/comparar', async (req, res) => {
    try {
        ...
        if (!snapshot) return res.status(404).json({ error: 'No hay snapshot para ese período' });

        // Construir mapa de snapshot: id_lote → { agente, resultadoTopeado, ... }
        const snapMap = new Map<number, { agente: string; resultadoTopeado: number; tipo: string; cantidad: number }>();
        for (const agentData of snapshot as any[]) {
        ...
        // Comparar
        const diffs: any[] = [];
        const soloEnBajada: any[] = [];
        const soloEnSnapshot: any[] = [];
...
router.post('/bajada/aplicar', async (req, res) => {
    try {
        ...
        const [bajadaMap, snapshot] = await Promise.all([
            readBajada(),
            loadMonthSnapshot(Number(year), Number(month)),
        ]);
        ...
        for (const agentData of snapshot as any[]) {
            ...
            if (agentNames && agentNames.length > 0 &&
                !agentNames.some((n: string) => n.toLowerCase() === agente.toLowerCase())) {
                continue;
            }

            const escala: number = agentData.escalaGen || 0;
            let componentePNuevo = 0;
            let cantidadActualizada = 0;
            const tropasActualizadas: any[] = [];
            const tropasNoEncontradas: number[] = [];
