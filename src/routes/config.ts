...
import { readSheet, writeSheet, appendSheet, clearSheetRange } from '../api/sheets';
...
router.post('/minimos', async (req, res) => {
    try {
        ...
        await appendSheet(config.HUB_SPREADSHEET_ID, "Config_Minimos!A:G", [row]);
...
router.post('/minimos/bulk', async (req, res) => {
    try {
        ...
        await appendSheet(config.HUB_SPREADSHEET_ID, "Config_Minimos!A:G", sheetRows);
...
router.put('/minimos/:id', async (req, res) => {
    try {
        ...
        await writeSheet(config.HUB_SPREADSHEET_ID, range, [row]);
        res.json({ success: true });
    } catch (e: any) {
        res.status(500).json({ error: e.message });
    }
...
router.delete('/minimos/:id', async (req, res) => {
    try {
        ...
        await clearSheetRange(config.HUB_SPREADSHEET_ID, "Config_Minimos!A2:G2000");

        if (raw.length > 0) {
            await writeSheet(config.HUB_SPREADSHEET_ID, `Config_Minimos!A2:G${raw.length + 1}`, raw);
        }
...
router.get('/escalas', async (req, res) => {
    try {
        const raw = await readSheet(config.HUB_SPREADSHEET_ID, "Config_Escalas!A2:G");
        const data: any[] = [];
        raw.forEach((row, idx) => {
            data.push({
                id: idx,
...
router.post('/escalas', async (req, res) => {
    try {
        ...
        await appendSheet(config.HUB_SPREADSHEET_ID, "Config_Escalas!A:G", [row]);
        res.json({ success: true });
    } catch (e: any) {
        res.status(500).json({ error: e.message });
    }
...
router.delete('/escalas/:id', async (req, res) => {
    try {
        ...
        await clearSheetRange(config.HUB_SPREADSHEET_ID, "Config_Escalas!A2:G2000");

        if (raw.length > 0) {
            await writeSheet(config.HUB_SPREADSHEET_ID, `Config_Escalas!A2:G${raw.length + 1}`, raw);
        }
...
router.get('/tajada', async (req, res) => {
    try {
        const raw = await readSheet(config.HUB_CIERRES_ID, "'Config_Tajada'!A2:J");
        const data: any[] = [];
        raw.forEach((row, idx) => {
            data.push({
                id: idx,
...
router.post('/tajada', async (req, res) => {
    try {
        ...
        await appendSheet(config.HUB_CIERRES_ID, "'Config_Tajada'!A:J", [row]);
        res.json({ success: true });
    } catch (e: any) {
        res.status(500).json({ error: e.message });
    }
...
router.delete('/tajada/:id', async (req, res) => {
    try {
        ...
        await clearSheetRange(config.HUB_CIERRES_ID, "'Config_Tajada'!A2:J2000");

        if (raw.length > 0) {
            await writeSheet(config.HUB_CIERRES_ID, `'Config_Tajada'!A2:J${raw.length + 1}`, raw);
        }
